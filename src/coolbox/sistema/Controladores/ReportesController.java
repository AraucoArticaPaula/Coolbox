...
public class ReportesController {
    ...
    @FXML
    private Label lblMontoVsPresupuesto;
    @FXML
    private Label lblEstadoPresupuesto;
    @FXML
    private Label lblTituloPresupuesto;
    @FXML
    private Label lblPorcentajeGarantias;
    @FXML
    private Label lblNumTransacciones;
    @FXML
    private Label lblLlenaLaBolsa;
    @FXML
    private Label lblTiendaPrincipal;

    @FXML
    private TableView<Comision> tblComisiones;
    @FXML
    private TableColumn<Comision, String> colTipoComision;
    @FXML
    private TableColumn<Comision, Double> colMontoComision;
    @FXML
    private TableColumn<Comision, String> colFechaComision;
    @FXML
    private TableColumn<Comision, String> colIdVentaComision;
    @FXML
    private TableColumn<Comision, Double> colMontoVenta;

    @FXML
    private TableView<ComisionResumen> tblAcumuladoComisiones;
    @FXML
    private TableColumn<ComisionResumen, String> colAcumuladoTipo;
    @FXML
    private TableColumn<ComisionResumen, Double> colAcumuladoTotal;
            ...
            CompletableFuture.supplyAsync(() -> {
                ...
                try (Connection con = ConexionDB.getConnection(); PreparedStatement stmt = con.prepareStatement(sql)) {
                    ...
                    try (ResultSet rs = stmt.executeQuery()) {
                        if (rs.next()) {
                            return rs.getString("nombre_tienda");
                        }
                    }
    ...
    private void loadEmpleadosSede(int idTienda) {
        CompletableFuture.supplyAsync(() -> {
            ...
            Platform.runLater(() -> cmbEmpleado.setItems(FXCollections.observableArrayList(empleados)));
        });
    }
    ...
    private void consultarReportes() {
        ...
        if ("Empleado".equals(enfoque) && cmbEmpleado.getValue() == null) {
            Alert alert = new Alert(Alert.AlertType.WARNING, "Seleccione un empleado válido para efectuar el análisis.", ButtonType.OK);
            alert.showAndWait();
            return;
        }
    ...
    private void procesarAnaliticaCompleta(LocalDate inicio, LocalDate fin, String enfoque, int empleadoId, int tiendaId) {
        // Envolvemos TODO el cálculo pesado en runAsync
        CompletableFuture.runAsync(() -> {
            ...
            StringBuilder sql = new StringBuilder(
                    "SELECT v.id_venta, v.monto, CONVERT(varchar, v.fecha, 120) AS f_venta, "+ "v.productos_por_venta, v.numero_boleta "+ "FROM VENTAS v WHERE CAST(v.fecha AS DATE) BETWEEN ? AND ? AND v.id_tienda = ?");
            if ("Empleado".equals(enfoque) && empleadoId > 0) {
                sql.append(" AND v.id_empleado = ").append(empleadoId);
            }
            sql.append(" ORDER BY v.id_venta DESC");

            try (Connection con = ConexionDB.getConnection(); PreparedStatement stmt = con.prepareStatement(sql.toString())) {
                ...
                final double finalTotalVentas = totalVentaSoles;
                
                if (llbIndicadorFinal > 2.00 && finalTotalVentas > 0 && "Empleado".equals(enfoque)) {
                    double bonoLLB = Math.round((finalTotalVentas * 0.05) * 100.0) / 100.0;
                    ...
                    cBono.setMontoVenta(finalTotalVentas);
                ...
                final int fTransacciones = totalTransacciones;
                ...
                Platform.runLater(() -> {
                    ...
                    lblNumTransacciones.setText(String.valueOf(fTransacciones));
                    lblLlenaLaBolsa.setText(String.format("%.2f", llbIndicadorFinal));
                    lblPorcentajeGarantias.setText(String.format("%.2f %%", tasaGarantias));
                });

                // Si es tienda, también calculamos el presupuesto
                if ("Tienda".equals(enfoque)) {
                    calcularPresupuestoTiendaCard(tiendaId, inicio, fin, finalTotalVentas);
                } else {
                    Platform.runLater(() -> {
                        lblMontoVsPresupuesto.setText(String.format("VENTAS: S/ %.2f", finalTotalVentas));
    ...
    private void calcularPresupuestoTiendaCard(int idTienda, LocalDate inicio, LocalDate fin, double ventasRealizadas) {
        // Este método ahora se ejecuta DENTRO del hilo secundario invocado por procesarAnaliticaCompleta,
        // por lo tanto la consulta de BD no congelará nada.
        ...
        try (Connection con = ConexionDB.getConnection(); PreparedStatement stmt = con.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    presupuestoMensual = rs.getDouble("meta");
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
        ...
        // Actualizamos la UI asegurando Platform.runLater()
        Platform.runLater(() -> {
