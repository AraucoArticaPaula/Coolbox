...
import javafx.application.Platform; // IMPORTANTE para la interfaz gráfica
...
import java.util.concurrent.CompletableFuture; // IMPORTANTE para la asincronía
...
public class SeguridadController {
    ...
    private void loadUsuarios() {
        CompletableFuture.supplyAsync(() -> {
            ...
            String sql = "SELECT u.id_usuario, u.id_empleado, (e.nombres + ' ' + e.apellidos) AS nombre_completo, "+ "u.nombre_usuario, u.correo, u.estado "+ "FROM USUARIOS u "+ "INNER JOIN EMPLEADOS e ON u.id_empleado = e.id_empleado";
            ...
            Platform.runLater(() -> tblUsuarios.setItems(FXCollections.observableArrayList(usuarios)));
        });
    }
    ...
    private void loadUsuarioRoles() {
        CompletableFuture.supplyAsync(() -> {
            ...
            String sql = "SELECT u.nombre_usuario AS usuario, r.nombre_rol AS rol "+ "FROM USUARIOS_ROLES ur "+ "LEFT JOIN USUARIOS u ON ur.id_usuario = u.id_usuario "+ "LEFT JOIN ROLES r ON ur.id_rol = r.id_rol";
            try (Connection connection = ConexionDB.getConnection(); PreparedStatement statement = connection.prepareStatement(sql); ResultSet rs = statement.executeQuery()) {
                while (rs.next()) {
            ...
            return usuarioRoles;
        }).thenAccept(usuarioRoles -> {
            Platform.runLater(() -> tblUsuariosRoles.setItems(FXCollections.observableArrayList(usuarioRoles)));
        });
    }
    ...
    private void loadRolesPermisos() {
        CompletableFuture.supplyAsync(() -> {
            ...
            String sql = "SELECT r.nombre_rol AS rol, p.nombre_permiso AS permiso "+ "FROM ROLES_PERMISOS rp "+ "LEFT JOIN ROLES r ON rp.id_rol = r.id_rol "+ "LEFT JOIN PERMISOS p ON rp.id_permiso = p.id_permiso";
            try (Connection connection = ConexionDB.getConnection(); PreparedStatement statement = connection.prepareStatement(sql); ResultSet rs = statement.executeQuery()) {
                while (rs.next()) {
            ...
            Platform.runLater(() -> tblRolesPermisos.setItems(FXCollections.observableArrayList(permisos)));
        });
    }
