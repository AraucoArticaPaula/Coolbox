...
public class PersonalController {
    ...
    private void initialize() {
        ...
        // Ejecutamos la búsqueda del nombre de la tienda en asíncrono
        // y le pasamos refreshAll() para que cargue las tablas al terminar.
        buscarYMostrarNombreTiendaActiva(this::refreshAll);
        ...
        tblEmpleados.getSelectionModel().selectedItemProperty().addListener((obs, oldVal, selected) -> {
            ...
            btnDeshabilitarEmpleado.setStyle(deshabilitado
                ? "-fx-background-color: #28A745; -fx-text-fill: white; -fx-background-radius: 6; -fx-font-weight: bold; -fx-cursor: hand;": "-fx-background-color: #FFC107; -fx-text-fill: #212121; -fx-background-radius: 6; -fx-font-weight: bold; -fx-cursor: hand;");
        });
    }
    ...
    private void buscarYMostrarNombreTiendaActiva(Runnable onComplete) {
    ...
    private void cambiarEstadoEmpleado(int idEmpleado, String nuevoEstado) {
        CompletableFuture.runAsync(() -> {
            try (Connection connection = ConexionDB.getConnection();
                ...
                e.printStackTrace();
                Platform.runLater(() -> mostrarAlerta("Error al actualizar el estado del empleado."));
            }
        }).thenRun(() -> {
        ...
        CompletableFuture.supplyAsync(() -> {
            ...
            String sql = esAdminGlobal
                    ? "SELECT e.id_empleado AS id, e.nombres, e.apellidos, e.DNI AS dni, e.celular, e.correo, e.tipo_empleado AS tipo, e.id_tienda, t.nombre_tienda, e.estado " +
                      ...
                      "WHERE ISNULL(e.estado, 'ACTIVO') <> 'BAJA'": "SELECT e.id_empleado AS id, e.nombres, e.apellidos, e.DNI AS dni, e.celular, e.correo, e.tipo_empleado AS tipo, e.id_tienda, t.nombre_tienda, e.estado " +
                      "FROM EMPLEADOS e INNER JOIN TIENDAS t ON e.id_tienda = t.id_tienda " +
            ...
            Platform.runLater(() -> tblEmpleados.setItems(FXCollections.observableArrayList(empleados)));
        });
    }
    ...
    private void loadHorarios() {
        ...
        // IMPORTANTE: Leemos la interfaz ANTES de iniciar el hilo asíncrono
        final LocalDate iniFinal = (dpHorarioInicio != null && dpHorarioInicio.getValue() != null) ? dpHorarioInicio.getValue() : LocalDate.now();
        final String diaFiltroFinal = (cmbHorarioDia != null && cmbHorarioDia.getValue() != null) ? cmbHorarioDia.getValue() : "Todos";
        final int diaSeleccionadoIndexFinal = iniFinal.getDayOfWeek().getValue();
        final LocalDate lunesSemanaFinal = iniFinal.with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY));
        final LocalDate domingoSemanaFinal = iniFinal.with(TemporalAdjusters.nextOrSame(DayOfWeek.SUNDAY));

        CompletableFuture.supplyAsync(() -> {
            List<Horario> horarios = new ArrayList<>();
            boolean esAdminGlobal = "ADMINISTRADOR".equalsIgnoreCase(SesionUsuario.getRolUsuario());

            StringBuilder sql = new StringBuilder("SELECT e.nombres + ' ' + e.apellidos AS empleado, h.dia_semana AS dia, h.hora_inicio AS inicio, h.hora_fin AS fin "+ "FROM HORARIOS h INNER JOIN EMPLEADOS e ON h.id_empleado = e.id_empleado "+ "WHERE h.fecha_inicio = ? AND h.fecha_fin = ?");
            ...
            try (Connection connection = ConexionDB.getConnection();
                ...
                int paramIndex = 1;
                statement.setDate(paramIndex++, java.sql.Date.valueOf(lunesSemanaFinal));
                statement.setDate(paramIndex++, java.sql.Date.valueOf(domingoSemanaFinal));
                ...
                try (ResultSet rs = statement.executeQuery()) {
                    while (rs.next()) {
                        ...
                        if (!"Todos".equals(diaFiltroFinal) && !diaFiltroFinal.equalsIgnoreCase(diaBD)) {
                            continue;
                        }
                        if (diaBDIndex < diaSeleccionadoIndexFinal) {
                            continue;
                        }
            ...
            Platform.runLater(() -> tblHorarios.setItems(FXCollections.observableArrayList(horarios)));
        });
    }
    ...
    private void loadCumplimiento() {
        ...
        CompletableFuture.supplyAsync(() -> {
            ...
            String sql = "SELECT e.nombres + ' ' + e.apellidos AS empleado, c.horas_semana AS horas, c.periodo, c.estado_validacion AS estado "+ "FROM CUMPLIMIENTO_HORAS c INNER JOIN EMPLEADOS e ON c.id_empleado = e.id_empleado";
        ...
        }).thenAccept(cumplimientos -> {
            Platform.runLater(() -> tblCumplimiento.setItems(FXCollections.observableArrayList(cumplimientos)));
        });
    }
