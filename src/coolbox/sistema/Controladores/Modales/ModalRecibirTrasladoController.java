...
public class ModalRecibirTrasladoController {
    ...
    @FXML private ComboBox<Empleado> cmbResponsableDestino;
    ...
    private void validarPermisoRecepcion() {
        // CORRECCIÓN: Si no es administrador, validamos usando el idTiendaUsuario numérico de su propia sesión activa
        if (!"ADMINISTRADOR".equalsIgnoreCase(SesionUsuario.getRolUsuario())) {
            // Nota: Ya no casteamos el string alfanumérico de destino. Confiamos en la consulta previa de la lista.
    ...
    private void completarDatosVista() {
        if (trasladoSeleccionado != null) {
            ...
            txtOrigen.setText(trasladoSeleccionado.getOrigen()); // Quitamos el prefijo duplicado
            txtCantidad.setText(String.valueOf(trasladoSeleccionado.getCantidad()));
        }
    }

    private void loadResponsablesDestino() {
        // CORRECCIÓN PROTEGIDA: Filtramos los empleados por el ID numérico de la tienda destino (del usuario logueado)
        if (idTiendaUsuario == -1) return;
        ...
        try (Connection connection = ConexionDB.getConnection();
            ...
            statement.setInt(1, idTiendaUsuario); // Enlazado al entero seguro de la sesión
    ...
    private void confirmarRecepcion() {
        ...
        if (empleado == null || trasladoSeleccionado == null || idTiendaUsuario == -1) return;
        ...
        try (Connection connection = ConexionDB.getConnection()) {
            ...
            // 1. Buscamos el ID del producto basándonos en el nombre del traslado seleccionado
            int idProducto = -1;
            ...
            // 2. Sumamos el stock en la tienda destino utilizando el ID seguro de la sesión
            ...
            // 3. Cambiamos el estado del traslado a 'Recibido'
    ...
    private int getInventarioId(Connection con, int idTienda, int idProd) throws SQLException {
        ...
        try (PreparedStatement stmt = con.prepareStatement(sql)) {
            stmt.setInt(1, idTienda); stmt.setInt(2, idProd);
    ...
    private void updateInventario(Connection con, int idInv, int stock) throws SQLException {
        ...
        try (PreparedStatement stmt = con.prepareStatement(sql)) {
            stmt.setInt(1, stock); stmt.setInt(2, idInv); stmt.executeUpdate();
        }
    }
    ...
    private void createInventario(Connection con, int idTienda, int idProd, int cant) throws SQLException {
        ...
        try (PreparedStatement stmt = con.prepareStatement(sql)) {
            stmt.setInt(1, idTienda); stmt.setInt(2, idProd); stmt.setInt(3, cant); stmt.executeUpdate();
        }
    }
