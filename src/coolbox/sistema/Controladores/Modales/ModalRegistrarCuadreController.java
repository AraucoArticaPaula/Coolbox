...
public class ModalRegistrarCuadreController {
    ...
    @FXML
    private TextField txtMontoEsperado;
    @FXML
    private TextField txtMontoTotal;

    @FXML
    private TextField txtFondoCaja;
    @FXML
    ...
    @FXML
    private TextField txtFechaCuadre;
        ...
        try (Connection connection = ConexionDB.getConnection()) {
            ...
            try (PreparedStatement stmtGlobal = connection.prepareStatement(sqlGlobal)) {
                ...
                try (ResultSet rs = stmtGlobal.executeQuery()) {
                    if (rs.next()) {
                        ventasGlobalTotal = rs.getDouble("total");
                    }
                }
            }
            ...
            try (PreparedStatement stmtTarjeta = connection.prepareStatement(sqlTarjeta)) {
                ...
                try (ResultSet rs = stmtTarjeta.executeQuery()) {
                    if (rs.next()) {
                        ventasTarjetaTotal = rs.getDouble("total_tarjeta");
                    }
                }
            }
    ...
    private void calcularDiferenciaCuadre() {
        ...
        if (Math.abs(diferencia) < 0.01) {
            ...
            lblAlertaMensaje.setText(String.format("DESCUADRE: Falta S/ %.2f en efectivo físico.", Math.abs(diferencia)));
