...
public class ModalRealizarTrasladoController {
    ...
    private void loadEmpleados() {
        StringBuilder sql = new StringBuilder(
                "SELECT e.id_empleado, e.nombres, e.apellidos FROM EMPLEADOS e "+ "INNER JOIN EMPLEADOS_CARGOS ec ON e.id_empleado = ec.id_empleado "+ "INNER JOIN CARGOS c ON ec.id_cargo = c.id_cargo "+ "WHERE c.nombre_cargo IN ('Gerente', 'Subgerente')");
    ...
    private void saveTraslado(Connection connection, int idProducto, int origen, int destino, int cantidad, int idEmpleado) throws SQLException {
        String sql = "INSERT INTO TRASLADOS(id_producto, id_tienda_origen, id_tienda_destino, cantidad, id_empleado, fecha, estado) "+ "VALUES(?, ?, ?, ?, ?, CAST(GETDATE() AS DATE), 'Pendiente')";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
