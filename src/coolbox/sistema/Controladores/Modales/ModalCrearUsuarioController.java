...
public class ModalCrearUsuarioController {
    ...
    private void loadEmpleadosSinUsuario() {
        ...
        String sql = esAdminGlobal
                ? "SELECT id_empleado, nombres, apellidos FROM EMPLEADOS WHERE id_empleado NOT IN (SELECT id_empleado FROM USUARIOS)": "SELECT id_empleado, nombres, apellidos FROM EMPLEADOS WHERE id_tienda = ? AND id_empleado NOT IN (SELECT id_empleado FROM USUARIOS)";
