...
import javafx.application.Platform; // Importado para UI Async
...
import java.util.concurrent.CompletableFuture; // Importado para asincronía
...
public class OperacionesController {
    ...
    private void initialize() {
        ...
        refreshAll(); // Inicializará los diccionarios y luego las tablas
    }

    private void precargarDiccionariosNombres(Runnable onComplete) {
        CompletableFuture.runAsync(() -> {
            ...
            // Ejecutar la acción posterior (ej. cargar tablas) en el hilo de JavaFX
            Platform.runLater(() -> {
                tblVentas.refresh(); // Refrescar celdas por si ya tenían IDs
                tblCuadres.refresh();
    ...
    private void refreshAll() {
        // Carga los nombres de IDs de fondo, y AL TERMINAR, carga las listas
    ...
    private void filtrarVentasPorFecha() {
        ...
        CompletableFuture.supplyAsync(() -> {
            ...
            StringBuilder sql = new StringBuilder(
                    "SELECT id_venta, id_tienda, id_empleado, CONVERT(varchar, fecha, 120) AS fecha, "+ "monto AS monto_total, productos_por_venta AS cantidad_productos, numero_boleta "+ "FROM VENTAS WHERE CONVERT(date, fecha) BETWEEN ? AND ?");
        ...
        }).thenAccept(ventasObtenidas -> {
            Platform.runLater(() -> tblVentas.setItems(FXCollections.observableArrayList(ventasObtenidas)));
        });
    }
    ...
    private void loadVentas() {
        CompletableFuture.supplyAsync(() -> {
            ...
            StringBuilder sql = new StringBuilder(
                    "SELECT id_venta, id_tienda, id_empleado, CONVERT(varchar, fecha, 120) AS fecha, "+ "monto AS monto_total, productos_por_venta AS cantidad_productos, numero_boleta FROM VENTAS");
    ...
    private void loadCuadres() {
        CompletableFuture.supplyAsync(() -> {
            ...
            return cuadres;
        }).thenAccept(cuadresObtenidos -> {
            Platform.runLater(() -> tblCuadres.setItems(FXCollections.observableArrayList(cuadresObtenidos)));
        });
    }
