...
import javafx.application.Platform; // Importante para la UI
...
import java.util.concurrent.CompletableFuture; // Importante para Asincronía
...
public class AlmacenController {
    ...
    private void initialize() {
        ...
        // Hacemos que se identifique al empleado primero (asíncrono)
        // y UNA VEZ termine, cargamos los datos dependientes.
    ...
    private void identificarUbicacionEmpleado(Runnable onComplete) {
        CompletableFuture.runAsync(() -> {
            String sql = "SELECT t.id_tienda, t.nombre_tienda FROM USUARIOS u "+ "JOIN EMPLEADOS e ON u.id_empleado = e.id_empleado "+ "JOIN TIENDAS t ON e.id_tienda = t.id_tienda "+ "WHERE u.nombre_usuario = ?";
    ...
    private void loadComboTiendas() {
        CompletableFuture.supplyAsync(() -> {
            ...
            Platform.runLater(() -> {
                ...
                // Buscar y seleccionar la tienda del usuario activo
                for (Tienda t : locales) {
    ...
    private void filtrarPorTienda() {
        ...
        CompletableFuture.supplyAsync(() -> {
            ...
            String sql = "SELECT p.id_producto, p.nombre, ISNULL(i.stock_actual, 0) AS cantidad_tienda "+ "FROM PRODUCTOS p "+ "LEFT JOIN INVENTARIO i ON p.id_producto = i.id_producto AND i.id_tienda = ?";
            ...
            return stockTienda;
        }).thenAccept(stockTienda -> {
            Platform.runLater(() -> tblInventario.setItems(stockTienda));
        });
    }
    ...
    private void loadMovimientos() {
        CompletableFuture.supplyAsync(() -> {
            ...
            StringBuilder sql = new StringBuilder(
                    "SELECT t.nombre_tienda, m.tipo_movimiento, m.cantidad, m.motivo, "+ "ISNULL(m.nombre_usuario, 'Sistema') AS nombre_usuario, "+ "ISNULL(CONVERT(VARCHAR, m.fecha, 120), 'Sin registro') AS fecha_movimiento "+ "FROM MOVIMIENTOS_INVENTARIO m "+ "JOIN TIENDAS t ON m.id_tienda = t.id_tienda");
            ...
            return movimientos;
        }).thenAccept(movimientos -> {
            Platform.runLater(() -> tblMovimientos.setItems(movimientos));
        });
    }
    ...
    private void loadTraslados() {
        CompletableFuture.supplyAsync(() -> {
            ...
            StringBuilder sql = new StringBuilder(
                    "SELECT t.id_traslado, p.nombre AS producto, "+ "orig.nombre_tienda AS origen, dest.nombre_tienda AS destino, "+ "t.cantidad, t.estado "+ "FROM TRASLADOS t "+ "LEFT JOIN PRODUCTOS p ON t.id_producto = p.id_producto "+ "LEFT JOIN TIENDAS orig ON t.id_tienda_origen = orig.id_tienda "+ "LEFT JOIN TIENDAS dest ON t.id_tienda_destino = dest.id_tienda");
            ...
            return traslados;
        }).thenAccept(traslados -> {
            Platform.runLater(() -> tblTraslados.setItems(traslados));
        });
    }
